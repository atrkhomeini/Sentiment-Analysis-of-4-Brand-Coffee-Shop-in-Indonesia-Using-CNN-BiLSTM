...
for idx, row in df.iterrows():
    ...
    prompt = f"""
    Tweet: "{tweet}"
    Available Aspects: Produk, Layanan, None.
    Question: Which aspect does the tweet talk about? Only answer one: Produk, Layanan, or None. Do not explain.
    """
    ...
    try:
        ...
        if counter % save_interval == 0:
            ...
            df.to_excel('labeled_n_aspect.xlsx', index=False)
...
with open(checkpoint_file, 'w') as f:
    json.dump(progress, f)
df.to_excel('../data/output/labeled_n_aspect.xlsx', index=False)
