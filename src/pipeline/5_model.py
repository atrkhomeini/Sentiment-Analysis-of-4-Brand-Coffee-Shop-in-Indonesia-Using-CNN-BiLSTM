...
balanced_df.to_excel('2_balanced_aspect_dataset.xlsx', index=False)
...
# Print header
print("n-fold,accuracy,precision,recall,f1,loss")
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X_train, y_train), 1):
    ...
    # Print fold results in CSV format with n-fold header
    print(f"fold:{fold},acc: {acc:.4f},precision: {precision:.4f},recall: {recall:.4f},f1: {f1:.4f},loss: {loss:.4f}")
